...
{
	private final static int DEFAULT_EVENTSUB_PORT = 8059;
	private final static int DEFAULT_SSDP_PORT = 8009;
	...
	private final ExecutorService mSSDPPacketPool = Executors.newCachedThreadPool();
	private synchronized void addDevice(final SSDPPacket ssdpPacket)
	{
		...
		if (dev != null) {
			if(ssdpPacket.getLocation().equals(dev.getLocation()))
				dev.setSSDPPacket(ssdpPacket);
			return;
		}
		//zhangbo@2013-8-8. parse device in a thread. 
		final WeakReference<ControlPoint> wr = new WeakReference<ControlPoint>(this);
		mSSDPPacketPool.execute(new Runnable() {
			@Override
			public void run() {
				if(null == wr.get())
					return;
				...
				if(null != cp)
					cp.addDevice(rootNode, ssdpPacket);
			}
		});
	}

	//zhangbo@2013-8-8. parse device in a thread. 
	private synchronized void addDevice(Node rootNode, SSDPPacket ssdpPacket)
	{
		...
		Device dev = getDevice(udn);
		if (dev != null) {
			if(dev.getTimeStamp() < ssdpPacket.getTimeStamp())
				dev.setSSDPPacket(ssdpPacket);
			return;
		}
	...
	{
		search(target, searchMx);
	}

	public void search()
	{
		search(ST.ROOT_DEVICE, searchMx);
	}
	...
	{
		return start(target, searchMx);
	}

	public boolean start()
	{
		return start(ST.ROOT_DEVICE, searchMx);
	}
